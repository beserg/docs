...
1. Выберите [зону доступности](../../overview/concepts/geo-scope.md), в которой будет находиться виртуальная машина, для работы примеров выберите ru-central1-a.
...
1. Выполните команду `sudo apt update`, чтобы обновить списки доступных для установки пакетов.
...
1. GitLab автоматически даст файлу имя `.gitlab-ci.yml` — не изменяйте его. Скопируйте в файл следующую конфигурацию:
   ...
   build:
     ...
     variables:
       ...
       folder_id: <идентификатор-каталога, можно посмотреть на главной странице консоли console.cloud.yandex.ru>
       subnet_name: <имя-подсети, где запущены машины, например default-ru-central1-a>
     script:
